...
export default function OddOrEvenComponent() {
    ...
    const getResult = async() => {
        if(input === ''){
            ...
            const promise = await OddOrEven(input);
            setResult(promise); 
        }   
    }
...
<div className="flex items-center">   
    <div className="relative">
        <input onChange={(e) => setInput(e.target.value)} value={input} type="text" className=" p-2.5 block w-[43px] sm:w-[117px] text-[15px] py-[2.5px] sm:text-[50px] rounded-lg sm:rounded-2xl border-none sm:py-[19px] RobotoFont" required/>
    </div>
    <button onClick={() => {
        getResult()
        setInput('')
