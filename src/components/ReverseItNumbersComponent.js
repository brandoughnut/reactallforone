...
import { ReverseItNumbers } from "../services/DataService.js";
...
export default function ReverseItNumbersComponent() {
    ...
    const getResult = async() => {
        if(input === ''){
            ...
            const promise = await ReverseItNumbers(input);
            setResult(promise); 
        }   
    }
  ...
  return (
    <div>
      ...
      <div className="text-center mb-8 sm:mb-16 RobotoFont">
        <p className="text-[30px] sm:text-[100px] mb-8 sm:mb-16">Reverse It Numbers</p>
        <p className="text-[15px] sm:text-[50px]">Please enter a number:</p>
      </div>
